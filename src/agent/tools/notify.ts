...
import { requireAuthority } from "./shared";
...
export function notifyTool(deps: AgentDeps) {
  return tool({
    ...
    callback: async (input, context) => {
      ...
      const client = sqlClient(deps.db);
      return client.begin(async (sql) => {
        ...
        if (input.visitId) {
          ...
          where id = ${input.visitId} and home_id = ${authority.homeId} and status <> 'cancelled'
        `;
          if (!visit)
        ...
        if (input.scheduledJobId) {
          ...
          where id = ${input.scheduledJobId}
            and home_id = ${authority.homeId} and status <> 'cancelled'
            and (${input.visitId ?? null}::uuid is null or visit_id = ${input.visitId ?? null})
        `;
          if (!job)
            throw new Error("Notification job is outside the task home");
        }
      ...
      `;
        ...
        const runId =
          typeof context?.invocationState.runId === "string"
            ...
            : null;
        await sql`insert into public.audit_events (home_id, run_id, actor, kind, payload)
          values (${authority.homeId}, ${runId}, 'agent', 'tool_call',
            ${JSON.stringify({ name: "notify", notificationId: row.id })}::jsonb)`;
