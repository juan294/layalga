...
describe("agent reschedule policy", () => {
  ...
  it("re-asks for changed approval and denies a children conflict", async () => {
    const vega = await createTemporaryHold(sql, clock, {
      invitationId: vegaInvitationId,
      stay: ["2026-09-18", "2026-09-21"],
      adults: 2,
      children: 2,
    });
    await confirmVisit(sql, clock, vega.visitId);
    const oteros = await createTemporaryHold(sql, clock, {
      invitationId: oterosInvitationId,
    ...
    const interrupted = await runAgentTask(
      {
        task: "guest_change",
        homeId,
        visitId: oteros.visitId,
        message: "26 to 28 September",
        locale: "en",
      },
      deps(model),
    );
    ...
    >`select id, interrupt_id from public.pending_decisions where run_id = ${interrupted.runId}`;
    ...
    const resumed = await runAgentTask(
      {
        ...
        responses: [
          {
            interruptId: pd!.interrupt_id,
            response: { approved: true, hostId },
          },
        ],
      },
      deps(model),
    ...
    const covered = await runAgentTask(
      {
        task: "guest_change",
        homeId,
        visitId: oteros.visitId,
        message: "Keep 26 to 28 September and wheelchair access",
        locale: "en",
      },
      deps(
        new ScriptedModel([
    ...
    expect(
      await sql`
        select id from public.pending_decisions where run_id = ${covered.runId}
      `,
    ).toHaveLength(0);
    const [coveredVisit] = await sql<{ approval_stay_hash: string | null }[]>`
      select approval_stay_hash from public.visits where id = ${oteros.visitId}
    `;
    expect(coveredVisit?.approval_stay_hash).toBe(after?.approval_stay_hash);

    const [party] = await sql<
      { id: string }[]
    >`insert into public.parties (home_id, family_name, locale, link_token) values (${homeId}, 'Third', 'en', ${crypto.randomUUID()}) returning id`;
    const [invite] = await sql<
      { id: string }[]
    >`insert into public.invitations (home_id, host_id, party_id, raw_message) values (${homeId}, ${hostId}, ${party!.id}, 'Third family') returning id`;
    const third = await createTemporaryHold(sql, clock, {
      invitationId: invite!.id,
      stay: ["2026-09-25", "2026-09-27"],
      adults: 1,
      children: 1,
    });
    ...
    const [verdict] = await sql<
      { payload: { tool: string; decision: string; reason: string } }[]
    >`
      select payload from public.audit_events
      where run_id = ${denied.runId} and kind = 'policy_verdict'
    `;
...
async function stay(visitId: string) {
  const [row] = await sql<
    { start: string; end: string }[]
  >`select lower(stay)::text as start, upper(stay)::text as end from public.visits where id = ${visitId}`;
