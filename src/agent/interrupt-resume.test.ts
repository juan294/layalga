...
describe("interrupt and resume", () => {
  ...
  it("resumes an approved tool exactly once in a new process", async () => {
    const first = await runAgentTask(submit(), deps(new ScriptedModel([
      { toolUse: { name: "create_temporary_hold", input: holdInput(invitationId) } },
      { text: "unused before restart" },
    ])));
    expect(first.status).toBe("interrupted");
    const [decision] = await sql<{ id: string; interrupt_id: string; interrupt_name: string; reason: { decision: string } | string }[]>`
      select id, interrupt_id, interrupt_name, reason from public.pending_decisions where run_id = ${first.runId}
    `;
    const reason = typeof decision?.reason === "string" ? JSON.parse(decision.reason) : decision?.reason;
    expect(reason?.decision).toBe("interrupt");
    expect(decision?.interrupt_name).toBe("host_decision");
    expect(await sql`select id from public.visits where home_id = ${homeId}`).toHaveLength(0);
    await recordDecision(decision!.id, "approved");

    const stdout = execFileSync(process.execPath, [
      "--import", "tsx", "src/agent/test-support/resume-child.ts",
      first.sessionId, decision!.interrupt_id,
      JSON.stringify({ approved: true, hostId }), homeId,
    ], { cwd: process.cwd(), env: { ...process.env, DATABASE_URL: url }, encoding: "utf8" });
    expect(JSON.parse(stdout).status).toBe("completed");
    const visits = await sql<{ status: string; approval_stay_hash: string | null }[]>`
    ...
    const audits = await sql<{ payload: { name: string } }[]>`
      select payload from public.audit_events where kind = 'tool_call'
        and payload->>'name' = 'create_temporary_hold'
    `;
    expect(audits).toHaveLength(1);
    const applied = await sql<{ payload: { pendingDecisionId: string; interruptId: string } }[]>`
    ...
    const [declinedDecision] = await sql<{ id: string; interrupt_id: string }[]>`
...
function submit(id = invitationId): AgentTask {
  return { task: "guest_submit", homeId, invitationId: id, stay: ["2026-09-19", "2026-09-21"], adults: 2, children: 0, pets: 1, notes: "wheelchair access", locale: "en" };
}
function holdInput(id: string) {
  return { invitationId: id, stay: ["2026-09-19", "2026-09-21"], adults: 2, children: 0, pets: 1, specialRequests: ["wheelchair access"] };
}
function deps(model: ScriptedModel) {
  return { db: sql, clock: new FakeClock(new Date("2026-09-07T08:00:00Z")), scheduler: new NoopScheduler(), appUrl: "http://localhost:3000", locale: "en" as const, model };
}
