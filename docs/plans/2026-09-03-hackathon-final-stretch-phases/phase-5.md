...
- [x] 5.1 ADR 0002 final addendum: production dispatch on AgentCore since the Phase 0 date, runtime ARN and version, memory id, tracing, rollback flag, and the 2026-09-03 `MODEL=bedrock` switch (verified by a live Okonkwo capture in 14.5 s). Note: no evidence of a "live Okonkwo capture in 14.5 s" was found anywhere in the repository or its history (no such party name exists in the demo seed, only Vega); the addendum records what is actually verifiable instead of inventing this detail — see the "Submission addendum: 2026-09-04" section of ADR 0002.
