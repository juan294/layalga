...
Guest prompt templates omit the stored family-name field. Host capture retains raw invitation text (`src/agent/run-task.ts:1188`) and is excluded from conversation extraction (`src/agent/memory.ts:38`). Its separate `recordCaptureMemory` event omits `partyName` and includes selected invitation facts (`src/agent/record-capture-memory.ts:89`). This reduces identity exposure but does not establish that no name reaches the provider or memory: guest change messages, selected structured text such as special requests, and tool content can still contain names. The provider prompt wrapper is not a general anonymizer, and extraction reads the conversation rather than only the provider-minimized text. Recall is tool-driven only (`search_memory`), never injected into the prompt, preserving the same prompt-minimization boundary described above; every successful `search_memory` call is audited as a `tool_call` row on the run, so recall is visible on the run timeline.
