...
No family name reaches memory or the model provider: `guest_submit`, `guest_change`, and `host_capture` prompts omit the family name at the source (`src/agent/run-task.ts`), so extraction never sees one, and the deterministic `recordCaptureMemory` write after a completed `host_capture` writes only invitation facts — party size, dates, arrival time, special requests, pets — never a name. Recall is tool-driven only (`search_memory`), never injected into the prompt, preserving the same prompt-minimization boundary described above; every successful `search_memory` call is audited as a `tool_call` row on the run, so recall is visible on the run timeline.
