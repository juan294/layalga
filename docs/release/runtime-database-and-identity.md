...
The AgentCore and web `DATABASE_URL` values use the Supabase pooler's **transaction-mode** port (`6543`), not session mode (`5432`). Each AgentCore microVM and each Vercel function instance can hold its own `postgres.js` pool, so concurrent instances can exceed a session-mode login's client cap. Transaction mode multiplexes these short-lived borrows over a smaller number of backend connections. The runtimes still use separate `layalga_agent` and `layalga_web` logins with their existing least-privilege grants.
