...
| -------------------------------------------------------------------------------------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `infra/iam/web-bedrock-policy.json`                                                          | `layalga-web` (IAM user)                                       | `bedrock:InvokeModel` on the selected Sonnet 4.5 inference profile and foundation models, `bedrock-agentcore:InvokeAgentRuntime` on `layalga_agent-*` runtimes, and an explicit deny on `GetWorkloadAccessTokenForUserId`      |
...
| `infra/iam/memory-data-plane.json` (on `feat/household-memory`, not yet merged to `develop`) | `layalga-web` and inline on `layalga-agentcore-runtime`        | The eleven `bedrock-agentcore:*` data-plane actions (`CreateEvent`, `ListMemoryRecords`, `BatchDeleteMemoryRecords`, and so on) on `arn:aws:bedrock-agentcore:us-east-1:106403001709:memory/LayalgaHouseholdMemory-CBgKZc7mK4` |

The web IAM user can invoke the model and the runtime and send host email; it never holds an AgentCore workload identity token. The runtime role can call the model and write its own traces and metrics; it does not hold SES or web-only database rights.
