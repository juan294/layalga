...
Model. The AgentCore runtime environment sets `MODEL=bedrock` explicitly, matching decision D4 of the final-stretch plan ("Production already runs `MODEL=bedrock`, switched and verified 2026-09-03"). This addendum cannot independently re-verify a specific timed capture beyond what the production runtime addendum above already records (the 2026-09-03 evidence run, run `c05759c1-49ae-4823-8b62-5482dae356a0`, and the tracing addendum's 22-second smoke capture on 2026-09-04); no additional timed capture evidence was found in the repository for this addendum, so none is asserted here.
