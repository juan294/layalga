...
## Release addendum: 2026-09-09 (v1.2.0)

Release v1.2.0 was tagged on `main` `0545ac0c27efd108dd279130a03f7b79d4a53b41`, the first and only candidate. No migrations were pending this cycle (schema-neutral release). The AgentCore runtime `layalga_agent-mONXXjFms4` was redeployed to version 24 (S3 object version `IPMCUXh_oWZILzGSGdAtMQPudOobOSNc`) from the tagged candidate; the prior bundle, version 23 (S3 object version `4ly9nxtGOYMf4_L95ETAZ_LGqMQW4z3Z`, deployed 2026-09-09 as the v1.1.0 hotfix build), is the rollback bundle.

The first probe attempt against production, run within minutes of the AgentCore redeploy, hit repeated timeouts: `src/agent/run-task.ts`'s `claimQueuedRun` only reclaims a `running` row after 6 minutes of staleness, and rapid back-to-back verification attempts (`demo:e2e` twice, then `release:probes` once) against the same fixed canonical demo home each collided with the previous attempt's still-valid claim, surfacing as CloudWatch's `[AGENTCORE_QUEUE_EXECUTION_FAILED]` alternating between "no longer active" and "already being executed" for the same run ID. This is a self-inflicted testing-cadence artifact, not a candidate defect — confirmed by re-running the full probe suite once, patiently, after a cooldown: all nine probes passed cleanly, including probe 5 (interrupt and resume, the exact scenario the collision was surfacing on) and probe 2 with a memory search on the capture run. Synthetic cleanup verified (`runs`/`visits`/`pending_decisions` at zero, `homes` and `invitations` at baseline). No code change was required.

## Release addendum: 2026-09-11 (v1.3.2)

Release v1.3.2 uses main `90b68385a590144d6d44cd7dd41298180b2d182c`. The web deployment and AgentCore runtime `layalga_agent-mONXXjFms4` version 30 both came from that commit; the runtime artifact is S3 object version `VaKvmW1EDGW4OEqhSXm_l6X6B4WR0_BB`. The release is schema-neutral and no migrations were pending.

Production verification found that the queued-run lifetime had two distinct clocks and two distinct process lifetimes. The direct AgentCore handler acknowledged an accepted run before its execution promise settled, so the invocation could finish before the worker. Keeping that request open exposed the second problem: claim eligibility, heartbeats, deadlines, and stale-run recovery used the household clock. A guided demo advances that clock by days, so recovery could reclaim an active real-time worker. Operational lease timestamps now use PostgreSQL `now()`. Household facts and scheduling behavior continue to use the injected household clock.

The [protected production workflow](https://github.com/juan294/layalga/actions/runs/34583050263) passed the guided demo and all nine probes against the exact deployed SHA. The host capture and interrupt-resume paths both recorded `executedOn = "agentcore"`; the capture searched memory; SES, concurrency, guest isolation, room coordination, and cleanup assertions passed. Final health reported zero stale runs, stale jobs, or retrying jobs.
