...
Date: 2026-09-04. Describes release v0.5.1.

This is an inventory of every Strands Agents SDK feature the project uses, with the file that uses it. Every claim below points at code. Snippets are verbatim.
...
The whole agent is built in one function, `buildAgent` in `src/agent/agent.ts`. Seven task kinds run through it: `host_capture`, `host_room_request`, `guest_submit`, `guest_change`, `guest_reconfirm`, `resume`, and `tick` (`src/agent/task.ts:22-91`). Each task is one `agent.invoke()` call inside a durable database run.
...
**How L'Ayalga uses it.** One factory builds every agent (`src/agent/agent.ts:44-68`):
...
followed by `systemPrompt`, `printer: false`, `toolExecutor: "sequential"`, `traceAttributes`, and a conditional `memoryManager` (`src/agent/agent.ts:54-68`). Two hooks are attached right after construction (`src/agent/agent.ts:69-70`).
...
- `systemPrompt` is chosen by locale, and a `resume` task gets a suffix that tells the model to write its summary in the deciding host's language (`src/agent/agent.ts:40-43`, `src/agent/system-prompt.ts:18-21`).
...
- `memoryManager` is spread in only when a store applies. `MemoryManager` throws with zero stores, so the option is omitted rather than passed empty (`src/agent/agent.ts:64-68`, `src/agent/memory.ts:114-128`).

**Why it matters.** One construction path means every task, test, and demo run gets the same hooks, the same storage, and the same trace attributes. There is no second agent for tests.
...
**How L'Ayalga uses it.** Production constructs `BedrockModel` from the environment and wraps it (`src/agent/agent.ts:74-85`):
...
`PromptMinimizingModel` extends `Model`, delegates `stateful`, `updateConfig`, and `getConfig`, and rewrites `TextBlock` content on the way into both `stream` and `countTokens` (`src/agent/prompt-minimization.ts:21-53`). The rewrite removes host and family names from prompt shapes still present in older session snapshots (`src/agent/prompt-minimization.ts:11-19`). New prompts already carry no names (`src/agent/run-task.ts:1181-1188`), so the wrapper is a second line, not the only one.

`ScriptedModel` also extends `Model` and yields the SDK's own stream event classes so the agent loop cannot tell it from a real provider (`src/agent/scripted-model.ts:34-87`):
...
`TaskScriptedModel` picks the next step by reading the latest `toolResultBlock` from the SDK `Message` history (`src/agent/scripted-model-selection.ts:33-64`, `257-278`). It is selected when `MODEL=scripted` (`src/agent/runtime/deps.ts:30-32`).
...
**How L'Ayalga uses it.** Ten tools, each in its own module under `src/agent/tools/`. Every one is a function of `deps` so the callback closes over the database, the clock, and the task authority.
...
Schemas carry hard bounds from `src/agent/task-limits.ts`, for example (`src/agent/tools/capture-invitation.ts:28-33`):
...
`find_visit_options` uses zod refinements to cap the search window at ninety days (`src/agent/tools/find-visit-options.ts:20-32`).

**Scoping per task.** `buildTools` gives a `host_room_request` only the three room tools and every other task the seven booking tools (`src/agent/deps.ts:20-37`). When a memory store applies, the SDK adds its own `search_memory` tool (section 7).

**Trusted inputs.** The model does not get to assert the facts that matter. Before a gated tool runs, the policy hook rewrites its input from server-side state (`src/agent/tools/shared.ts:88-91`):
...
For `create_temporary_hold`, the stay, party size, special requests, room choice, and overflow consent all come from the guest's own submission, held in `deps.authority.guestSubmission`, not from the model (`src/agent/tools/shared.ts:92-108`, `src/agent/tools/create-temporary-hold.ts:41-48`). `approvedBy` is set only by the hook after a host approval (section 4).

**Authority checks.** Every callback resolves the home through `requireAuthority`, `homeIdForInvitation`, or `homeIdForVisit`, which throw when a record is outside the task's home or outside the one invitation or visit the task is scoped to (`src/agent/tools/shared.ts:29-75`). `notify` also enforces deterministically that a party may only ever receive a `reconfirm_chase` (`src/agent/tools/notify.ts:129-138`).

**`invocationState`.** Every tool audits itself with the run id the SDK threads through `context.invocationState` (`src/agent/tools/shared.ts:19-22`). `prepare_room_action` refuses to run without it (`src/agent/tools/prepare-room-action.ts:30-35`).

**Bounded outputs.** Room tools truncate labels and arrangements before returning them to the model (`src/agent/tools/room-output.ts:9-19`).
...
**How L'Ayalga uses it.** The policy hook gates three tools (`src/agent/policy-hook.ts:22-26`) and is installed on every agent (`src/agent/agent.ts:69`). It runs in this order (`src/agent/policy-hook.ts:38-127`):
...
A second hook audits the SDK's own `search_memory` tool, which none of the project's tool modules can see (`src/agent/memory.ts:141-148`):
...
**Why it matters.** The system prompt tells the model it never decides whether a host must be asked (`src/agent/system-prompt.ts:2`). The hook is where that promise is enforced. The model cannot skip it, cannot pass its own `approvedBy`, and sees only a cancel message or a completed tool result.
...
**How L'Ayalga uses it.** The hook raises the interrupt with a typed response and a structured reason (`src/agent/policy-hook.ts:82-94`):
...
const response = event.interrupt<HostDecision>({
  ...
  reason: (overflowInterrupt
    ...
    : hostDecisionReason(draft, verdict)) as unknown as JSONValue,
});
...
The reason carries the requested draft and a `stayApprovalHash` (`src/agent/host-decision-context.ts:21-47`). The host UI never trusts that JSON blindly: `verifiedHostDecisionContext` re-validates every field and recomputes the hash before showing the decision (`src/agent/host-decision-context.ts:50-118`).

The run loop persists the interrupt (`src/agent/run-task.ts:405-437`): each entry of `result.interrupts` becomes a `pending_decisions` row keyed by session id and interrupt id, and the run is marked `interrupted`.

A `resume` task carries the host's responses (`src/agent/task.ts:66-85`). Before the agent is built, the run checks each decision was recorded by that host with that note (`src/agent/run-task.ts:329-355`) and claims it with `applied_run_id` (lines 356-369). The invoke input is then a list of SDK response objects (`src/agent/run-task.ts:379-385`):
...
**Exactly once.** On completion the run writes a `decision_applied` audit row guarded by a not-exists check (`src/agent/run-task.ts:439-467`). On failure it releases `applied_run_id` only when that row was never written (lines 498-516), so a retry can apply the decision but a second apply cannot. If a resumed stay still matches the approval hash, the hook does not interrupt again (`src/agent/policy-hook.ts:80`).
...
**How L'Ayalga uses it.** `PostgresStorage` implements `Storage` over the `agent_sessions` table (`src/agent/storage/postgres-storage.ts:23-87`):
...
Keys are normalized and `..` is rejected (lines 5-16); `list` escapes `LIKE` wildcards (lines 19-21, 61-75). `namespace("session")` returns a prefixed view (lines 77-81), which is how `buildAgent` mounts it (`src/agent/agent.ts:47-53`). `saveLatestOn: "message"` writes after every message, so an interrupt is on disk before the process returns.

**Session ids.** `resolveSessionId` derives a stable id per conversation (`src/agent/run-task.ts:902-921`): `inv_<invitationId>` for every guest task on one invitation, `tick_<jobId>`, `capture_<hostId>`, `room_<hostId>`, and the original id for `resume`. The SDK validates ids with `/^[a-z0-9_-]+$/` (`node_modules/@strands-agents/sdk/dist/src/session/validation.js:10`). ADR 0002 records why the planned `inv:` prefix became `inv_` (`docs/decisions/0002-agent-runtime.md:20`). The same id is reused as the AgentCore Memory session id (section 7).
...
**How L'Ayalga uses it.** `memoryStoresForTask` decides per task which store applies and whether it may write (`src/agent/memory.ts:53-100`):
...
The actor id is `home-<homeId>` or `home-<homeId>/party-<partyId>`, so one party can never read another party's records (`src/agent/memory.ts:65-68`, `82`).

`memoryConfigForTask` fixes the manager options (`src/agent/memory.ts:122-127`):
...
`injection: false` keeps recall tool-driven, so the prompt shapes the minimizer expects stay intact. `addToolConfig: false` means the model has no direct write path. The `search_memory` description is the project's own (`src/agent/memory.ts:22-23`).

**Extraction boundary.** `InvocationTrigger` reads the whole conversation. A `host_capture` conversation contains the host's raw message, which names the family, so that task is never extraction-backed (`src/agent/memory.ts:37-46`). The one write path for what a capture teaches the house is `recordCaptureMemory`, which calls AgentCore `CreateEvent` directly with a name-free text and `clientToken: runId` so a retried run never double-writes (`src/agent/record-capture-memory.ts:27-71`).

**Flush.** Extraction runs in the background. At the end of every invoke the run calls `agent.memoryManager?.flush()` inside a guard that logs and swallows failure, because a memory write must never fail the booking run it rides with (`src/agent/run-task.ts:62-73`, `403`).

**Prompt steers.** When memory is on, the prompt asks the model to call `search_memory` first and states that recalled facts never change party size, dates, or special requests (`src/agent/run-task.ts:1127-1128`). `capture_invitation` enforces that deterministically: a special request that merely restates a `rememberedContext` entry is dropped (`src/agent/tools/capture-invitation.ts:62-73`).

**Why it matters.** The house remembers arrival habits, pets, and room needs across visits, but a family's name never enters the memory service and no party can see another's records.
...
**How L'Ayalga uses it.** The agent span carries ids only (`src/agent/agent.ts:59-63`):
...
**Why it matters.** A judge or an operator can open one trace and see the task, the home, the model call, and every tool call, without any personal name in the span attributes.
...
**How L'Ayalga uses it.** The AgentCore entrypoint is seventeen lines (`src/agent/runtime/agentcore.ts:5-17`):
...
`handleAgentCoreRequest` accepts three envelopes (`src/agent/runtime/handler.ts:45-127`): `execute_run` for a run the web side already queued, `scheduled_tick` from EventBridge Scheduler, and a bare task that is awaited. For `execute_run`, `acceptAgentRunExecution` registers an async task, starts the run in the background, and returns `{ status: "accepted", runId }` immediately (`src/agent/runtime/async-execution.ts:21-35`). The web client treats that acknowledgement as accepted work, not a result, and the terminal state is read from the run row (`src/agent/client.ts:64-78`).

Every invoke is bounded (`src/agent/run-task.ts:390-396`):
...
The four-minute budget matches the run's claim deadline (`src/agent/run-task.ts:563`), so a cancelled invoke and a lease that expires are the same event to the queue. `invocationState.runId` is what every tool and the policy hook use to audit against the right run (section 3).
...
- **Booking state.** Tools call `src/core/booking/*` functions; the model never writes a row. The room choice, overflow consent, and approver id are stripped from tool input and re-supplied from server state (`src/agent/tools/shared.ts:88-108`).
- **Policy decisions.** `evaluateOverlap` and `evaluateRoomSelection` in `src/core/policy` and `src/core/rooms` decide allow, interrupt, or deny. The hook applies the verdict; the system prompt tells the model it does not (`src/agent/system-prompt.ts:2`, `src/agent/policy-hook.ts:53-59`).
- **Approval.** A host decision lives in `pending_decisions`, is recorded through the host UI, and a resume verifies the recorded status, host, and note before the SDK ever sees the response (`src/agent/run-task.ts:329-355`).
...
- **Queueing, retries, and idempotency.** Runs are rows in `runs` with an intent key, a claim token, a heartbeat, and a deadline; a cron drain claims stale leases (`src/agent/run-task.ts:556-592`, `src/agent/queue.ts:23-97`). The SDK's own background tasks are not used for durability.
- **Memory writes for captures.** `add_memory` is off and `host_capture` has no writable store; the one capture write is a direct, name-free `CreateEvent` (section 7).
- **Names in prompts and traces.** New prompts carry ids only, the model wrapper strips older shapes, and trace attributes hold ids (`src/agent/run-task.ts:1181-1188`, `src/agent/prompt-minimization.ts`, `src/agent/agent.ts:57-63`).
- **Tenant boundaries.** Every tool re-checks the home and, where scoped, the single invitation or visit of the task (`src/agent/tools/shared.ts:29-75`); the AgentCore runtime connects as a non-owner database role (ADR 0002).
...
| --------------------------------------- | ------------------------------------------------------------ | --------------------------------------------------------------- |
| `Agent` with full option set            | `src/agent/agent.ts`                                         | One construction path for every task, test, and runtime         |
| `BedrockModel`                          | `src/agent/agent.ts`                                         | Claude Sonnet 4.5 through Bedrock in production                 |
...
| `tool()` with zod schemas               | `src/agent/tools/*.ts`                                       | Ten bounded, tenant-checked, self-auditing tools                |
...
| `InterruptResponseContent`              | `src/agent/run-task.ts`                                      | Resume with verified host responses, applied exactly once       |
