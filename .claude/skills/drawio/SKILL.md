...
3. **If the user requested an export format** (png, svg, pdf), locate the draw.io CLI (see below) and export with `--embed-diagram`. **Always keep the source `.drawio` file** — it is committed alongside its export and is the file that gets edited and diffed. If the CLI is not found, keep the `.drawio` file and tell the user they can install the draw.io desktop app to enable export, or open the `.drawio` file directly
...
- Keep the `.drawio` source after export. Never delete it. Editing XML in place and re-exporting the paired file is the intended workflow
