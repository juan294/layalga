...
Before acting, read `docs/release/e2e-pro-playbook.md` completely. It is the
single project-specific release procedure and may declare release blocked. This
command remains the only tagging and publication authority, but invoking it does
not itself authorize deployment, push, tag creation, or publication. Obtain each
authorization required by the playbook at the named gate.
