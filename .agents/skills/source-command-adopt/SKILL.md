---
name: "source-command-adopt"
description: "Migrated source command `adopt`"
---

# source-command-adopt

Use this skill when the user asks to run the migrated source command `adopt`.
...
- Does AGENTS.md exist? If so, read it fully. Does it bridge Codex to the existing `.Codex/*` structure?
- Does `.Codex/settings.json` exist? What permissions and env vars are configured?
- Does `.Codex/commands/` exist? Which slash commands are present?
- Does `.Codex/skills/` exist? Which skills are installed?
- Does `.Codex/rules/` exist? Which rules are installed? (New in blueprint v2)
- Does `.Codex/agents/` exist?
- Is `.Codex/settings.local.json` in `.gitignore`?
- Does AGENTS.md contain `<important if>` blocks? (These should migrate to `.Codex/rules/`)
...
- Missing `.Codex/skills/` or missing blueprint-provided skills
- Missing `.Codex/rules/` (conditional and modular rules)
- AGENTS.md contains `<important if>` blocks (should be `.Codex/rules/` with `paths`)
...
- `.Codex/settings.local.json` not gitignored
...
- AGENTS.md exists but is missing operational rules
...
1. AGENTS.md (affects every session)
2. AGENTS.md (makes the same workflow operable in Codex)
3. settings.json + Agent Teams (affects Codex agent capabilities)
...
5. **Create or update `AGENTS.md`** — adapt from `cc-rpi/templates/AGENTS.md.template`:
   - Point Codex at `AGENTS.md`, `.Codex/commands/`, `.Codex/rules/`, and `.Codex/skills/`
...
6. **Install `.Codex/rules/`** — copy rule templates from `cc-rpi/templates/rules/`:
...
7. **Migrate `<important if>` blocks** — if the project's AGENTS.md has `<important if>` blocks:
   - Extract each block's content
   - Create a `.Codex/rules/` file with `paths` frontmatter
   - Remove the block from AGENTS.md
...
- **Merge, don't replace.** If AGENTS.md already exists with useful content, add the missing pieces — don't replace the whole file.
...
- **Keep AGENTS.md lean.** Target ~70 lines. Domain rules go in `.Codex/rules/` and `.Codex/skills/`, not AGENTS.md.
