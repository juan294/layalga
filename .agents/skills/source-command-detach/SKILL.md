---
name: "source-command-detach"
description: "Migrated source command `detach`"
---

# source-command-detach

Use this skill when the user asks to run the migrated source command `detach`.
...
You are cleanly removing cc-rpi artifacts from this project. The blueprint lives at `/Users/juan/code/cc-rpi/`.
...
1. Check for `.Codex/cc-rpi-sync.json` or RPI commands in `.Codex/commands/` (research.md, plan.md, implement.md, validate.md).
...
- `.Codex/commands/describe-pr.md`
- `.Codex/commands/pre-launch.md`
- `.Codex/commands/status.md`
- `.Codex/commands/fix-ci.md`
- `.Codex/commands/explore-release.md`
- `.Codex/hooks/guard-bash.sh`
- `.Codex/cc-rpi-sync.json`
- `scripts/agents/cc-rpi-update.sh` (nightly sync agent, if exists)

For each command file that exists, diff it against `/Users/juan/code/cc-rpi/templates/commands/<name>` to detect customization. Mark as "unmodified" or "customized."

For `AGENTS.md`, diff it against `/Users/juan/code/cc-rpi/templates/AGENTS.md.template` to detect customization. Mark as "unmodified" or "customized."
...
Read the project's AGENTS.md (or AGENTS.md for copilot-rpi projects) and identify these blueprint-managed sections by their `##` or `###` headers:
...
Read `.Codex/settings.json` and identify:
...
3. Clean Tier 3 configuration:
   - Remove the `PreToolUse` hook entry for guard-bash.sh from `.Codex/settings.json`. If no other hooks remain, remove the entire `hooks` key.
...
5. Clean up empty directories: remove `.Codex/commands/` if empty, `.Codex/hooks/` if empty. Do NOT remove `.Codex/` itself or `.Codex/settings.json`.
...
Removed RPI methodology commands, hooks, AGENTS.md blueprint sections,
...
Removed: [N] files, [N] AGENTS.md sections, [N] settings.json entries
...
- **Don't touch Codex itself.** `.Codex/` directory, `settings.json` (with remaining entries), and `settings.local.json` are Codex's own -- they exist independently of cc-rpi.
