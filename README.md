...
The v1.3.4 release safely formats agent run summaries, removes emoji from public results, and keeps internal interrupt payloads out of the run page. The broader coordination behavior was proven in production as v1.3.2 at commit `90b68385a590144d6d44cd7dd41298180b2d182c`: the protected workflow ran the guided demo and all nine probes against the exact Vercel and AgentCore candidate, including AgentCore Memory, host SES acceptance, concurrency, human interruption, guest isolation, and cleanup. Human time savings have not been measured; the video, final Devpost submission, guest-email activation, and real-recipient delivery proof remain separate work. Three supporting AWS Builder Center articles are published and linked from the [judge guide](docs/submission/judge-guide.md).
